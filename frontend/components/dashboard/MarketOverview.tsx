  ...
  return (
    <div className="space-y-6">
        ...
        {overview.indices.map((index) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">{index.symbol}</p>
      ...
      <div className="rounded-2xl border border-white/10 bg-slate-950/80 p-5 ring-1 ring-white/5">
        <div className="mb-3 rounded-xl border border-amber-500/20 bg-amber-950/25 px-3 py-2 text-xs text-amber-100/95">{liveHint}</div>
        <div className="mb-4 flex items-center justify-between">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">Xu hướng gần đây</p>
          <span className="text-xs text-slate-500">VNINDEX · preview</span>
        </div>
        <div className="h-64">
        ...
        {overview.sector_heatmap?.length ? (
          <div className="mt-5 border-t border-white/5 pt-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Sector heatmap</p>
